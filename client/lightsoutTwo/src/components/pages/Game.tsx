...
// Define interface for legacy pattern mapping
...
const Game = () => {
    ...
    const fetchPatterns = async () => {
      ...
      try {
        ...
        if (response.data && response.data[""]) {
          ...
          console.log(
            "Found classic difficulty levels:",
            response.data["classic"]
          );
          ...
          console.log(
            "No empty or classic difficulties found in response:",
            response.data
          );
  ...
  const linearToRC = useCallback(
    (linearIndex: number, size: number): [number, number] => {
  ...
  const rcToLinear = useCallback(
    (row: number, col: number, size: number): number => {
  ...
  const setupPuzzle = useCallback(
    (newGrid: boolean[][], gameLevel: number) => {
      ...
      console.log(
        `Setting up puzzle for level ${gameLevel}, game mode: ${gameMode}`
      );
      console.log(
        `Available classic patterns:`,
        Object.keys(classicPatterns).map(Number)
      );
      ...
      if (gameMode === "classic") {
        ...
        if (classicPatterns[gameLevel]) {
          console.log(
            `Found server pattern for level ${gameLevel}:`,
            classicPatterns[gameLevel]
          );
        ...
        // Fallback to legacy patterns
          ...
          console.log(
            `Using legacy pattern for level ${gameLevel}:`,
            legacyPatterns[gameLevel]
          );
      ...
      console.log(
        `No pattern applied, generating random puzzle for level ${gameLevel}`
      );
  ...
  const initializeGame = useCallback(
    (size: number, gameLevel: number) => {
      console.log("initializeGame start:", {
        size,
        gameLevel,
        serverPattern: classicPatterns[gameLevel],
      });
      // Create a new grid
      const newGrid: boolean[][] = Array(size)
        .fill(false)
  ...
  }, [gameMode, level, /* toggleCell */ gridSize, linearToRC]);
  ...
  useEffect(() => {
    if (gameMode !== "classic") return;
    console.log("Patterns-loaded effect:", {
      isLoadingPatterns,
      classicPatterns,
      gameMode,
      gridSize,
    ...
    if (
      !isLoadingPatterns &&
      Object.keys(classicPatterns).length > 0 &&
      gameMode === "classic"
    ) {
  ...
  // Compute minimum moves using solver for random and classic modes
  useEffect(() => {
    // only run when in random or classic and we have an initial grid
    if (
      (gameMode !== "random" && gameMode !== "classic") ||
      !initialGrid.length
    )
      return;
    ...
    setMinMoves(null);
    apiClient
      .post("/puzzles/validate", { pattern })
      .then((res) => {
        console.log("Puzzle minMoves:", res.data.minimumMoves);
        setMinMoves(res.data.minimumMoves);
      })
      .catch((err) => console.error("Failed to validate puzzle", err));
  ...
  const handleCellClick = (row: number, col: number) => {
    ...
    const hasWon = newGrid.every((row: boolean[]) =>
              ...
              <h2>
                {minMoves != null
                  ? moveCount === minMoves
                    ...
                    : moveCount <= minMoves + 4
                    ? "Almost perfect! Level Complete!"
                    : "Level Complete!"
                  : "Level Complete!"}
              </h2>
              <p>
                You completed level {currentLevel} in {moveCount} moves.
              </p>
              {gameMode === "custom" ? (
                <button
